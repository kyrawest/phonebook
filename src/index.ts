...
type SortableContactField = Extract<
  keyof Contact,
...
class PhoneBook {
  contacts: Contact[] = [];
  sortedBy: SortableContactField | false = false;
  ...
  public search(
    phoneNumber: number,
    startIndex: number = 0,
    stopIndex: number = this.contacts.length
  ): { contact: Contact; index: number } | null {
  ...
  public update(
    phoneNumber: number,
    fieldName: SortableContactField,
    newValue: string
  ): void {
  ...
  public delete(phoneNumber: number): void {
  ...
  public sort(sortBy: SortableContactField = "phoneNumber"): void {
    ...
    if (
      !["firstName", "lastName", "fullName", "phoneNumber", "email"].includes(
        sortBy
      )
    ) {
      throw new Error(
        `Invalid sort field: ${sortBy}. Valid fields are: firstName, lastName, fullName, phoneNumber, email.`
      );
    }

    // If the phonebook is already sorted by the specified field, do nothing.
    if (this.sortedBy === sortBy) return;
  ...
  private mergeSort(
    arr: Contact[] = this.contacts,
    sortBy: SortableContactField = "phoneNumber"
  ): Contact[] {
  ...
  private mergeByPhone(left: Contact[], right: Contact[]): Contact[] {
    let sorted: Contact[] = [];
    let leftIndex = 0;
    let rightIndex = 0;
  ...
  private mergeByString(
    left: Contact[],
    right: Contact[],
    sortBy: Exclude<SortableContactField, "phoneNumber">
  ): Contact[] {
    let sorted: Contact[] = [];
    let leftIndex = 0;
    let rightIndex = 0;
    ...
    while (leftIndex < left.length && rightIndex < right.length) {
      if (
        left[leftIndex][sortBy].localeCompare(right[rightIndex][sortBy]) <= 0
      ) {
        sorted.push(left[leftIndex]);
        leftIndex++;
      } else {
...
phonebook.add(
  Contact6.firstName,
  Contact6.lastName,
  Contact6.phoneNumber,
  Contact6.email
);
phonebook.add(
  Contact7.firstName,
  Contact7.lastName,
  Contact7.phoneNumber,
  Contact7.email
);
phonebook.add(
  Contact8.firstName,
  Contact8.lastName,
  Contact8.phoneNumber,
  Contact8.email
);
...
console.log(
  "Phonebook after updating John Doe's first name to Johnathan",
  phonebook.search(1234567890).contact
);
